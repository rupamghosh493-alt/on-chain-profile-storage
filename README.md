...
A fully functional, decentralized Web3 application built on the Stellar Soroban blockchain. This dApp allows users to securely store, retrieve, and search for personal profiles (Name and Bio) on-chain, featuring a sleek, premium dark-themed frontend and an optimized Rust smart contract.
...
*   **Premium UI:** A beautiful, responsive, and modern dark-themed frontend built dynamically with HTML, CSS, and vanilla JavaScript.
...
2.  **Frontend (`/frontend`)**: A pure HTML/CSS/JS Web3 application that natively integrates with the `@stellar/stellar-sdk` and `@stellar/freighter-api` to interact with the deployed contract on the Soroban Testnet.
...
The frontend uses vanilla standard web technologies for ultimate performance and beginner friendliness.

1. Navigate to the frontend directory from the root:
   ```bash
   cd frontend
   ```
2. Start a local web server (you can use whatever static server you prefer):
   ```bash
